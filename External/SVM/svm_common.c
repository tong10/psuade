...
/* added by Charles Tong */
void gen_documents(char *docfile, DOC ***docs, double **label, 
		   long int *totwords, long int *totdoc, int nInputs,
                   int nTrains, double *trainInputs, double *trainOutputs)
{
  char *line,*comment;
  WORD *words;
  int  ii;
  ...
  comment = (char *)malloc(10*sizeof(char));
  words = (WORD *)my_malloc(sizeof(WORD)*(max_words_doc+10));
  if(verbosity>=1) {
    printf("Reading examples into memory..."); fflush(stdout);
  }
  ...
  while(dnum < nTrains) {
    ...
    for (ii=0; ii<nInputs;ii++){
       words[ii].weight = trainInputs[dnum*nInputs+ii];
       words[ii].wnum = ii+1;
    }
#endif
