C **********************************************************************
C This file was sent to me upon my request to Professor M. Powell
C in early March 2009. This file did not come with any copyright
...
C Reference:
C    M.J.D. Powell (2009), "The BOBYQA algorithm for bound constrained
C    optimization without derivatives", Report No. DAMTP 2009/NA06,
C    Centre for Mathematical Sciences, University of Cambridge, UK.
C **********************************************************************
C Changes made: 
C     - take out the main program section
C     - function update changed to updateb
C     - create FMODE common variable to choose between MOO and Bobyqa
C **********************************************************************
C
...
C     DIMENSION X(100),XL(100),XU(100),W(500000)
C     TWOPI=8.0D0*DATAN(1.0D0)
C     BDL=-1.0D0
C     BDU=1.0D0
C     IPRINT=2
C     MAXFUN=500000
...
C     DO 50 JCASE=1,2
C     NPT=N+6
C     IF (JCASE .EQ. 2) NPT=2*N+1
C     PRINT 30, M,N,NPT
C  30 FORMAT (//5X,'2D output with M =',I4,',  N =',I4,
C    1  '  and  NPT =',I4)
C     DO 40 J=1,M
C     TEMP=DFLOAT(J)*TWOPI/DFLOAT(M)
C     X(2*J-1)=DCOS(TEMP)
C  40 X(2*J)=DSIN(TEMP)
C     CALL BOBYQA (N,NPT,X,XL,XU,RHOBEG,RHOEND,IPRINT,MAXFUN,W,CALFUN)
C  50 CONTINUE
C     M=M+M
C     IF (M .LE. 10) GOTO 10
C     STOP
C     END
C
C
...
C     DO 10 I=4,N,2
C     DO 10 J=2,I-2,2
C     TEMP=(X(I-1)-X(J-1))**2+(X(I)-X(J))**2
C     TEMP=DMAX1(TEMP,1.0D-6)
C  10 F=F+1.0D0/DSQRT(TEMP)
C     RETURN
C     END
C
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% calfun.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
C
      CALL UPDATEB(N,NPT,BMAT,ZMAT,NDIM,VLAG,BETA,DENOM,KNEW,W)
      IH=0
      PQOLD=PQ(KNEW)
      PQ(KNEW)=ZERO
      DO 470 I=1,N
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% update.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE UPDATEB(N,NPT,BMAT,ZMAT,NDIM,VLAG,BETA,DENOM,
     1  KNEW,W)
      IMPLICIT REAL*8 (A-H,O-Z)
