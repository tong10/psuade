...
{
   int    maxF=10000, nInps=4, ii, iOne=1;
   ...
   for (ii = 0; ii < nInps; ii++)
     printf("OUU Bobyqa initial X %d = %12.4e\n",ii+1,X[ii]);
   opt = new OUUOptimizer();
   for (ii = 0; ii < nInps; ii++)
      //opt->setVariableType(ii+1, OUUDesignCont);
      opt->setVariableType(ii+1, 1);
   opt->setObjectiveFunction(evalF);
   psConfig_.InteractiveOn();
   opt->optimize(nInps, X, lbnds, ubnds, iOne, maxF, tol);
   optX = opt->getOptimalX();
   for (ii = 0; ii < nInps; ii++)
     printf("OUU Bobyqa final   X %d = %12.4e\n",ii+1,optX[ii]);
   printf("Optimal Y = %e\n", opt->getOptimalY());
   delete opt;
   delete [] X;
   delete [] lbnds;
