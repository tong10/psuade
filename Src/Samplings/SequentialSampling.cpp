...
int SequentialSampling::initialize(int initLevel)
{
  ...
  double ddata, *sampleInputs=NULL, *SY, quality[3];
  ...
  if (sampleMatrix_ != NULL)
  {
    ...
    rsPtr->setBounds(lowerBounds_, upperBounds_);
    rsPtr->setOutputLevel(printLevel_);
    sampleInputs = new double[nSamples_*nInputs_];
    for (ii = 0; ii < nSamples_; ii++)
      for (jj = 0; jj < nInputs_; jj++)
        sampleInputs[ii*nInputs_+jj] = sampleMatrix_[ii][jj];
    status = rsPtr->initialize(sampleInputs,sampleOutput_);
  ...
  else
  {
    ...
    for (ii = 0; ii < nInputs_; ii++)
    {
      sprintf(pString,"Enter hyperparameter for input %d : ", ii+1);
      gpParams[ii] = 0.0;
  ...
  printf("1. PSUADE is to create a candidate design set\n");
  printf("2. You will provide a candidate design set, or\n");
  ...
  if (kk == 1)
  {
    ...
    sampler->setInputBounds(nInputs_, lowerBounds_, upperBounds_);
    sampler->setOutputParams(1);
    sampler->setSamplingParams(nTests, 1, 0);
    sampler->initialize(0);
    sampleInputs = new double[nTests*nInputs_];
    SY = new double[nTests];
    SS = new int[nTests];
    sampler->getSamples(nTests, nInputs_, 1, sampleInputs, SY, SS);
    delete [] SS;
    delete [] SY;
    delete sampler;
    ...
    for (ii = 0; ii < nTests; ii++)
      for (jj = 0; jj < nInputs_; jj++)
        testMatrix.setEntry(ii,jj,sampleInputs[ii*nInputs_+jj]);
    delete [] sampleInputs;
  }
  else
  {
  ...
  if (nSamples_ > 0)
  {
    ...
    {
      ddata = 0.0;
      for (jj = 0; jj < nSamples_; jj++) ddata += sampleMatrix_[jj][ii];
      XMeans_[ii] = ddata / (double) nSamples_;
      ddata = 0.0;
      for (jj = 0; jj < nSamples_; jj++)
        ddata += pow(sampleMatrix_[jj][ii] - XMeans_[ii], 2.0);
  ...
  sampleInputs = new double[nDesigns*nInputs_];
  for (jj = 0; jj < nDesigns; jj++)
  {
    for (ii = 0; ii < nInputs_; ii++)
    {
      ddata = finalDesign.getEntry(jj,ii);
      sampleInputs[jj*nInputs_+ii] = ddata;
    }
  }
  SamplingQuality(nDesigns, nInputs_, sampleInputs, lowerBounds_,
                  upperBounds_, quality);
...
int SequentialSampling::genDesignsUltimate(psVector &params, 
                               ...
                               int nDesigns, psMatrix &finalDesign)
{
  int    ii, jj, kk, nn, nTests, count, keepFlag, *indices, *bestIndices;
  int    iter;
  double ddata, dist, meas, minVal, maxVal;
  FILE   *fp=NULL;
  ...
  nTests = testMatrix.nrows();
  indices = new int[nTests];
  for (ii = 0; ii < nTests; ii++) indices[ii] = 0;
  bestIndices = new int[nTests];
  for (ii = 0; ii < nTests; ii++) bestIndices[ii] = 0;
  minVal = PSUADE_UNDEFINED;
  iter = 0;
  evaluateUltimate(nTests,indices,0,nDesigns,nDesigns,params,
                   testMatrix,bestIndices,minVal,iter);
...
// ------------------------------------------------------------------------
int SequentialSampling::evaluateUltimate(int nTests, int *indices,
                 int position, int nDesigns, int K, psVector &params, 
                 psMatrix &testMatrix, int *bestIndices, double &minVal,
                 int &iter)
{
  ...
  localIndices = new int[nTests];
  for (ii = 0; ii < nTests; ii++) localIndices[ii] = indices[ii];
  doEval = 0;
  if (K == 0)
  {
    doEval = 1;
  ...
  {
    for (ii = 0; ii < K; ii++) localIndices[position+ii] = 1;
    doEval = 1;
  }
  ...
  if (doEval == 1)
  {
    ...
    {
      minVal = maxVal;
      for (nn = 0; nn < nTests; nn++) bestIndices[nn] = localIndices[nn];
  ...
  evaluateUltimate(nTests, localIndices, position+1, nDesigns, K, params, 
                   testMatrix, bestIndices, minVal, iter);
  localIndices[position] = 1;
  evaluateUltimate(nTests, localIndices, position+1, nDesigns, K-1, params, 
                   testMatrix, bestIndices, minVal, iter);
  delete [] localIndices;
  return 0;
}
