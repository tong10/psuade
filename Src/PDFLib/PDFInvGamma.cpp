...
int PDFInvGamma::getPDF(int length, double *inData, double *outData)
{
  ...
  if (psConfig_.PDFDiagnosticsIsOn())
    printf("PDFInvGamma: getPDF ends.\n");
  return 0;
}
...
int PDFInvGamma::getCDF(int length, double *inData, double *outData)
{
  ...
  if (psConfig_.PDFDiagnosticsIsOn()) printf("PDFInvGamma: getCDF ends.\n");
  return 0;
}
...
{
  int    ii;
  double ddata, mult, xlo, xmi, xhi, ylo, ymi, yhi;
  ...
  for (ii = 0; ii < length; ii++)
  {
    ...
    {
      printf("PDFInvGamma invCDF ERROR - CDF value %e not in (0,1).\n",
             ddata);
      exit(1);
    }
    xlo = 1.0e08;
  ...
  if (psConfig_.PDFDiagnosticsIsOn()) printf("PDFInvGamma: invCDF ends.\n");
  return 0;
}
