...
int PDFLogNormal::getPDF(int length, double *inData, double *outData)
{
  ...
  if (psConfig_.PDFDiagnosticsIsOn()) printf("PDFLogNormal: getPDF ends\n");
  return 0;
}
...
int PDFLogNormal::getCDF(int length, double *inData, double *outData)
{
  ...
  if (psConfig_.PDFDiagnosticsIsOn()) 
    printf("PDFLogNormal: getCDF ends\n");
  return 0;
}
...
{
  int    ii;
  double ddata, xlo, xhi, ylo, yhi, xmi, ymi, iroot2;
  ...
  for (ii = 0; ii < length; ii++)
  {
    ...
    {
      printf("PDFLogNormal invCDF ERROR - CDF value not in (0,1).\n");
      exit(1);
    }
    xlo = 1.0e-8;;
    ylo = 0.5 * (1.0 + erf((log(xlo)-mean_)*iroot2));
    xhi = 10.0 * stdev_;
  ...
  if (psConfig_.PDFDiagnosticsIsOn())
    printf("PDFLogNormal: invCDF ends.\n");
  return 0;
}
                            ...
                            double *uppers)
{
  ...
  //**/ -------------------------------------------------------------
  //**/ lower bounds has to be > 0 and upper > lower
  //**/ -------------------------------------------------------------
