 ...
 *  \file PrintingTS.h
 *
 *  \brief utility routines for printing output to stdout, stderr, and 
 *         log files.
 */
 ...
 *  printLevel gives level of output to allow.  The higher the number, 
 *  the more output is printed.
 ...
 *    logFileName may be null if only printing (not logging) is to be 
 *    enabled. 
 * Calling initializePrintingTS is not necessary if you do not plan 
 *   to do logging. 
 * If initilizePrinting is not called, output to terminal will occur, 
 *   and the default 
 * print level of 0. 
 *-----------------------------------------------------------------*/
void initializePrintingTS(int printLevel, const char* logFileName, 
                          int myRank);
...
/** \brief Return the current printLevel (See description in 
 * initilaizePrintingTS) */
/*---------------------------------------*/
int getPrintLevelTS(void);

/** \brief Sets the current printLevel. */
/*  printLevel gives level of output to allow.  The higher the 
 *  number, the more output is printed. */
/*  0 only prints fatal errors  */
/*  1 prints warnings and fatal errors */
/*  2 prints output of interest for later viewing (non-interactive) */
/*  3 and above prints everything (for interactive users */
