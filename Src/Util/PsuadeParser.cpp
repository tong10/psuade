  ...
  while (idata < lLimit || idata > uLimit)
  {
    ...
    if (status >= 0)
    {
      ...
      if (status < 0) idata = lLimit - 1;
        ...
        printf("ERROR: input %d not within range [%d,%d].\n",
                idata,lLimit,uLimit);
      delete [] tokens;
    }
    ...
    if (count > 10)
    {
      printf("getInt ERROR: bad data entered more than 10 times.\n");
      printf("       ABORT.\n");
      exit(1);
    }
  }
...
double getDouble(char *inString)
{
  ...
  while (status < 0)
  {
    ...
    if (count > 10)
    {
      printf("getDouble ERROR: bad data entered more than 10 times.\n");
      printf("          ABORT.\n");
      exit(1);
    }
  }
...
int getString(char *inString, char *cstr)
{
  ...
  while (status < 0)
  {
    ...
    if (count > 10)
    {
      printf("getString ERROR: bad data entered more than 10 times.\n");
      printf("          ABORT.\n");
      exit(1);
    }
  }
