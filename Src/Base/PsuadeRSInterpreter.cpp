...
int PsuadeBase::RSBasedAnalysis(char *lineIn, PsuadeSession *session)
{
   ...
   else if (!strcmp(command, "rsua"))
   {
      ...
      if (uaMethod == 0)
      {
         ...
         if (discFile == 1)
         {
            ...
            {
               ddata = faPtrsRsEval[1]->evaluatePointFuzzy(&userSamInps[ss*nInputs],
                                                           discSamStd);
         ...
         if (fp != NULL)
         {
            fprintf(fp,"%% This file is primarily for diagnostics and expert analysis\n");
            ...
            fclose(fp);
            printf("The outputs and stds of your sample has been written to 'rsua_sample'.\n");
         }
         ...
         else
         {
            ...
            if (dsum == 0.0)
            {
               ...
               fprintf(fp,"text(0.01,0.5,'No RS uncertainty info.','sc','FontSize',11)\n");
               fprintf(fp,"text(0.01,0.4,'Deterministic RS used.','sc','FontSize',11)\n");
            }
            else
            {
               fprintf(fp,"subplot(2,2,3)\n");
   ...
   else if (!strcmp(command, "rssobol2b"))
   {
      ...
      printf("Quantities are normalized. Multiply by variance to compute actual.\n");
      printEquals(PL_INFO, 0);
      for (ii = 0; ii < nInputs; ii++)
      {
   ...
   else if (!strcmp(command, "rssoboltsib"))
   {
      ...
      if (psPlotTool_ == 1)
      {
         ...
         if (fp == NULL) printf("ERROR : cannot open file scilabrssoboltsib.sci\n");
         else
         {
      ...
      else
      {
         ...
         if (fp == NULL) printf("ERROR : cannot open file matlabrssoboltsib.sci\n");
         else
         {
            fprintf(fp,"%% This file contains total order Sobol' indices\n");
            fprintf(fp,"%% with error bars coming from bootstrapping.\n");
   ...
   else if (!strcmp(command, "rsb_ua"))
   {
      ...
      if (winput[0] == 'y')
      {
         ...
         printf("1. use bootstrapping + RS means (or deterministic RS, e.g. MARS)\n");
         printf("2. use stochastic RS (Kriging, MARS+Bootstrap, regression)\n");
         printf("3. use (2) but performing worst-case analysis ((2) - average case)\n");
      ...
      if (uaMethod == 2)
      {
         ...
         if (fp != NULL)
         {
            ...
            fclose(fp);
            printf("The outputs and stds of your sample has been written to 'rsbua_sample'.\n");
         }
   ...
   else if (!strcmp(command, "rs_qsa2"))
   {
      ...
      {
         printf("NOTE: Some inputs have non-uniform distributions - will be used.\n");
         pdfman = new PDFManager();
         pdfman->initialize(psuadeIO);
      ...
      for (kk = 0; kk < numBS; kk++)
      {
         ...
         for (ii1 = 0; ii1 < nInputs; ii1++)
         {
            ...
            for (ii2 = ii1+1; ii2 < nInputs; ii2++)
            {
               ...
               {
                  bin1 = (int) ((OAInputs[kk2*nInputs+ii1]-iLowerB[ii1]+1.0e-12)/width1);
                  bin2 = (int) ((OAInputs[kk2*nInputs+ii2]-iLowerB[ii2]+1.0e-12)/width2);
               ...
               for (kk2 = 0; kk2 < nOA; kk2++)
               {
                  bin1 = (int) ((OAInputs[kk2*nInputs+ii1]-iLowerB[ii1]+1.0e-12)/width1);
                  bin2 = (int) ((OAInputs[kk2*nInputs+ii2]-iLowerB[ii2]+1.0e-12)/width2);
                  ievars[bin1*nOA1+bin2] += pow(OAOutputs[kk2]-iemeans[bin1*nOA1+bin2],2.0);
               }
               for (kk2 = 0; kk2 < nOA1*nOA1; kk2++)
               ...
               ievar = 0.0;
               for (kk2 = 0; kk2 < nOA1*nOA1; kk2++) ievar += pow(iemeans[kk2]-iemean,2.0);
