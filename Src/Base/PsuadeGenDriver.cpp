...
int PsuadeBase::genBatchFile(int genFlag)
{
   ...
   fprintf(fp,"###Replace the following line with the correct calling sequence\n");
...
int PsuadeBase::genDriver(int genFlag)
{
   ...
   if (nFiles > 0)
   {
      sprintf(pString,"Where can your application template be found(absolute path):");
      getString(pString, response);
   ...
   if (nSupportFiles > 0)
   {
      sprintf(pString,"Where can these support files be found(absolute path):");
      getString(pString, response);
      response[strlen(response)-1] = '/';
      response[strlen(response)] = '\0';
   ...
   fprintf(dfp,"   nCols   = string.split(lineIn)\n");
   ...
   fprintf(dfp,"            sInd = string.find(newLine, inputNames[fInd])\n");
   ...
   fprintf(dfp,"      lineLen = len(lineIn)\n");
   fprintf(dfp,"      newLine = lineIn\n");
   fprintf(dfp,"      sInd    = string.find(newLine, searchString)\n");
   fprintf(dfp,"      if sInd > 0:\n");
   fprintf(dfp,"         next = sInd + strLen\n");
   ...
   fprintf(dfp,"\" \" + newLine[next:lineLen+1]\n");
   fprintf(dfp,"         newLine = lineTemp\n");
   fprintf(dfp,"      sInd = string.find(newLine, searchString2)\n");
   fprintf(dfp,"      if sInd > 0:\n");
   ...
   fprintf(dfp,"#     sIndex = string.find(lineIn, \"%s\")\n", userName);
   fprintf(dfp,"#     if sIndex > 0:\n");
   fprintf(dfp,"#        sIndex = string.find(lineIn, lastJob)\n");
   fprintf(dfp,"#        if sIndex > 0:\n");
   ...
   fprintf(dfp, "            if lineIn == \"\":\n");
   fprintf(dfp, "               break\n");
   fprintf(dfp, "            sIndex = string.find(lineIn, batchFile)\n");
   fprintf(dfp, "            if sIndex > 0:\n");
